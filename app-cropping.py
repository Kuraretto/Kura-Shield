...
from mtcnn import MTCNN # <--- NEW: Advanced Detector
...
try:
    ...
    st.error(f"Error loading MTCNN: {e}. Try 'pip install mtcnn'")
    st.stop()
...
# ==========================================
# HELPER FUNCTIONS (CROP & BG REMOVAL)
# ==========================================
...
def get_face_crop(image_array, is_video_frame=False, padding=0.3, min_conf=0.9, remove_bg=False):
    """
    Advanced Face Cropping using MTCNN + Optional GrabCut
    """
    try:
        # 1. Ensure RGB format for MTCNN
        if is_video_frame:
        ...
        # 2. Run MTCNN Detection
        results = mtcnn_detector.detect_faces(img_rgb)
        
        if not results:
            return image_array, False # Fallback to full image

        # 3. Filter & Find Best Face
        valid_faces = [r for r in results if r['confidence'] >= min_conf]
        if not valid_faces:
             return image_array, False
             
        # Pick largest face or highest confidence
        best_face = max(valid_faces, key=lambda x: x['confidence'])
        x, y, w, h = best_face['box']

        # 4. Fix Coordinates & Add Padding
        x, y = max(0, x), max(0, y)
        x_pad = int(w * padding)
        y_pad = int(h * padding)
        ...
        # 5. Crop
        cropped_rgb = img_rgb[y_start:y_end, x_start:x_end]
        
        # 6. Remove Background (Optional)
        if remove_bg:
            cropped_rgb = remove_background_grabcut(cropped_rgb)

        # 7. Return in original format
        if is_video_frame:
            # Convert back to BGR for video writing
            return cv2.cvtColor(cropped_rgb, cv2.COLOR_RGB2BGR), True
        else:
...
def load_real_detector():
    ...
    if missing_models:
        return None, None, f" Missing Image Models: {', '.join(missing_models)}"
    try:
...
def process_full_video_pipeline(video_file, detector, selected_features, padding, min_conf, remove_bg):
    st.subheader("2. Full Video Forensics")
    save_dir = get_output_path("video", video_file.name)
    st.markdown(f"📂 **Output Folder:** `{save_dir}`")
    st.markdown("Processing frames with **MTCNN Face Detection**.")
    ...
    # Video Warning for GrabCut
    if remove_bg:
        st.warning("⚠️ 'Remove Background' is active. This is very slow for videos!", icon="⏳")
    ...
    while cap.isOpened():
        ...
        # --- MTCNN FACE CROP ---
        # Note: remove_bg on video is slow and might flicker, but user requested logic.
        cropped_frame, face_found = get_face_crop(frame, is_video_frame=True, padding=padding, min_conf=min_conf, remove_bg=remove_bg)
...
def process_image_analysis(cropped_img_pil, filename, padding, min_conf, remove_bg):
    st.subheader("2. Ensemble Analysis Results")
    save_dir = get_output_path("image", filename)
    st.markdown(f"📂 **Output Folder:** `{save_dir}`")
    
    img_array_raw = np.array(cropped_img_pil.convert('RGB'))
    
    # --- NEW: MTCNN Auto-Crop ---
    img_array, face_found = get_face_crop(img_array_raw, is_video_frame=False, padding=padding, min_conf=min_conf, remove_bg=remove_bg)
    
    col_disp1, col_disp2 = st.columns(2)
    with col_disp1:
        st.image(img_array_raw, caption="Original Input", width=200)
    with col_disp2:
        if face_found:
            st.success("✅ MTCNN Face Detected")
            st.image(img_array, caption="Processed Face Input", width=200)
        else:
            st.warning("⚠️ Face detection failed. Using full image.")
            st.image(img_array, caption="Fallback Input", width=200)
...
def main():
    st.title("🛡️ Deepfake Forensics Lab")
    ...
    st.sidebar.markdown("Adjust how the AI detects faces.")
    
    padding_val = st.sidebar.slider("Face Padding", 0.0, 0.5, 0.3, 0.05, help="Amount of background (hair/chin) to keep.")
    min_conf = st.sidebar.slider("Min Confidence", 0.5, 1.0, 0.9, 0.05, help="Higher = Less false positives.")
    remove_bg = st.sidebar.checkbox("Remove Background (GrabCut)", value=False, help="Black out background pixels. Slow on video!")
    ...
    if mode == "Image":
        ...
        if uploaded_file:
            ...
            st.image(original_image, caption="Original Input", width=400)
            
            if st.button("Analyze Image"):
                process_image_analysis(original_image, uploaded_file.name, padding_val, min_conf, remove_bg)
        ...
        if uploaded_video:
            ...
            if st.button("Start Full Forensics Analysis"):
                process_full_video_pipeline(uploaded_video, detector, selected_features, padding_val, min_conf, remove_bg)
        ...
        if not audio_h5 and not audio_tflite:
            ...
            uploaded_audio = st.file_uploader("Upload Audio/Video", type=['wav', 'mp3', 'flac', 'ogg', 'mp4', 'avi', 'mov'])
            if uploaded_audio:
