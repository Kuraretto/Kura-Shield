...
# Page Configuration
st.set_page_config(page_title="Deepfake Forensics Lab (CPU Mode)", layout="wide")
...
class DeepfakeDetector:
    def __init__(self, input_shape=(224, 224, 3)):
        ...
        # OPTIMIZATION: n_jobs=-1 uses all CPU cores, tree_method='hist' is faster on CPU
        self.xgb = XGBClassifier(n_jobs=-1, tree_method='hist') 
...
if error_msg:
    ...
    # CPU Confirmation Message
    st.success("✅ Application successfully configured to run on CPU.", icon="🖥️")
...
def main():
    st.title(" Deepfake Forensics Lab (CPU Mode)")
